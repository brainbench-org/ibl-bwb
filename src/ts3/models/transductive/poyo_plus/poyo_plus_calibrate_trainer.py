...
class POYOPlusCalibrateTrainer(POYOPlusMultitaskPretrain):
    ...
    def link_model(self, model: POYOPlus):
        # train and val share one dataset (one recording, split only at the sampler),
        # so the transform is only ever set once, but the loop stays split-shaped to
        # match every other trainer's link_model and avoid a silent divergence later.
        ...
        model.link_datasets(
            self.train_loader.dataset, self.val_loader.dataset, finetune_enable=True
        )
