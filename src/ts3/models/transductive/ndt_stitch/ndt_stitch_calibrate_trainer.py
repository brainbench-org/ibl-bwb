"""Calibration: continue NDT-Stitch's masked spike modelling objective on one recording.

This trainer is used for TS3 transductive evaluation for NDT-Stitch, wherein we generate a
brain region prediction by first calibrating the model on the new session, creating an
...
:class:`~ts3.models.transductive.NDTStitchExtractor` reads as the eval part of an embeddings
file, which a probe in ``ts3/eval.py`` then classifies into brain regions.

This trainer directly extends the NDT-Stitch pretrain trainer
:class:`~pretrain.models.ndt_stitch.NDTStitchPretrain`. Three things have to be added on top
of it. ``setup_model`` instantiates with ``finetune_enable=True``. Its loaders read
``IBLBrainWideBenchMaskModelingSpikes``, which hardcodes ``regime="pretrain"`` and a
``pretrain_{split}_domain`` no eval recording carries, so ``setup_train_loader`` /
``setup_val_loader`` build from :class:`~ts3.models.transductive.calibration_dataset.TrialAlignedCalibrationDataset` instead, one
recording split causally in time. ``link_model`` calls ``NDTStitch.load_ckpt``: the pretrain
trainer accepts a ``ckpt`` argument and currently ignores it, being only ever run once from scratch,
so without this the pretrained backbone would never be read and the fresh stitchers would
...
from pretrain.models.ndt_stitch import NDTStitch, NDTStitchMasker, NDTStitchPretrain
from ts3.models.transductive.calibration_dataset import TrialAlignedCalibrationDataset


class NDTStitchCalibrateTrainer(NDTStitchPretrain):
