...
class POYOSingleTaskPretrain(BaseTrainer):
    ...
    def loss(self, pred, X, y):
        target = y["values"]
        if self.readout_spec.target_resolution == TargetResolution.TIMESTEP:
            # (B, T, D) -> (B, T*D)
        ...
        if "mask" in y:
            assert self.readout_spec.target_resolution == TargetResolution.TIMESTEP, (
    ...
    def predict(self, X, target_timestamps):
        output = self.model(
            **X["model_inputs"],
        ...
        if self.readout_spec.target_resolution == TargetResolution.TIMESTEP:
            # queries are chained, so restore the batch axis of the target: (B, T, D)
            return output.view(*target_timestamps.shape, -1)
        return output
    ...
    def train_epoch(self):
        ...
        for X, y in loader:
            ...
            with torch.autocast(device_type="cuda", dtype=self.precision.dtype):
                pred = self.predict(X, y.get("timestamps"))
